(() => {
  ...
  const roleLabel=(value)=>({customer:'Заказчик',contractor:'Исполнитель',buyer:'Покупатель',seller:'Продавец',tenant:'Арендатор',landlord:'Арендодатель',other:'Другое'}[String(value||'')]||'не указана');
  const focusLabel=(value)=>({money:'деньги',terms:'сроки',liability:'ответственность',termination:'расторжение',hidden:'скрытые обязательства',all:'все риски'}[String(value||'')]||'все риски');
  ...
  function setupCheck(){
    ...
    form.addEventListener('submit',(event)=>{
      ...
      const data=new FormData(form);const role=String(data.get('role')||''),focus=String(data.get('focus')||''),signed=String(data.get('signed')||'');
      window.proverjdoGoal?.('check_requested',{product:'document_check',role,focus,signed});
      saveDraft({source_text:text,role,focus,signed,created_at:new Date().toISOString()});
  ...
  function setupPayment(){
    ...
    form.addEventListener('submit',async(event)=>{
      ...
      try{
        ...
        window.proverjdoGoal?.('payment_started',{product:'document_check',product_id:'contract_check_490',order_id:String(data.order_id),amount:490});
        sessionStorage.setItem('proverjdo.payment.v1',JSON.stringify({order_id:data.order_id,scan_id:draft.scan_id,email}));
        localStorage.setItem(RESULT_ORDER_KEY,String(data.order_id));
        location.assign(data.payment_url);
      }catch(e){window.proverjdoGoal?.('payment_failed',{product:'document_check',product_id:'contract_check_490',error_code:String(e?.code||'PAYMENT_ERROR')});if(error){error.textContent=`${e?.code||'PAYMENT_ERROR'} · ${e?.message||'Не удалось создать платёж.'}`;error.classList.remove('hidden')}submit.disabled=false;}
    });
  }
  ...
  async function setupProverjdoLogin(){
    ...
    localStorage.setItem(RESULT_ORDER_KEY,orderId);
    try{
      await loadSdk();
      ...
      if(session){location.replace('../result/');return;}
      const form=byId('proverjdo-login-form'),email=byId('proverjdo-login-email'),error=byId('proverjdo-login-error'),success=byId('proverjdo-login-success');
      try{const saved=JSON.parse(sessionStorage.getItem('proverjdo.payment.v1')||'null');if(saved?.email&&!email.value)email.value=saved.email}catch{}
      form?.addEventListener('submit',async(event)=>{
        event.preventDefault();if(!form.reportValidity())return;
  ...
  function renderPaidResult(payload){const result=payload.result||{};byId('result-risk').textContent=`Риск: ${riskLabel(result.risk_level)}`;byId('result-summary').textContent=result.summary||'Разбор завершён.';byId('result-context').textContent=`Роль: ${roleLabel(payload.role)} · Фокус: ${focusLabel(payload.focus)} · Договор подписан: ${payload.signed==='yes'?'да':'нет'}`;const findings=byId('result-findings');findings.replaceChildren();for(const [index,item] of (Array.isArray(result.findings)?result.findings:[]).entries()){const card=document.createElement('article');card.className='risk-card paid-risk-card';const head=document.createElement('div');head.className='paid-risk-head';const number=document.createElement('span');number.className='risk-number';number.textContent=`Риск №${index+1}`;const h3=document.createElement('h3');h3.textContent=item.title||'Риск';head.append(number,h3);const why=document.createElement('p'),ws=document.createElement('strong');ws.textContent='Почему это важно. ';why.append(ws,document.createTextNode(item.why||''));const action=document.createElement('p'),as=document.createElement('strong');as.textContent='Что сделать. ';action.append(as,document.createTextNode(item.action||''));card.append(head,why,action);findings.append(card)}if(!findings.children.length){const box=document.createElement('div');box.className='panel';box.textContent='Существенных рисков не найдено.';findings.append(box)}fillList('result-missing',result.missing_terms);fillList('result-questions',result.questions);fillList('result-checklist',result.checklist);byId('result-status').classList.add('hidden');byId('result-content').classList.remove('hidden');byId('result-heading').textContent='Полный разбор договора'}
  async function setupPaidResult(){if(document.body.dataset.page!=='paid-result')return;try{await loadSdk();const client=window.supabase.createClient(config.url,config.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});const query=new URLSearchParams(location.search);let orderId=query.get('order_id')||query.get('InvId');if(!orderId){try{orderId=localStorage.getItem(RESULT_ORDER_KEY)}catch{}}if(!orderId||!/^\d+$/.test(orderId))throw new Error('ORDER_NOT_FOUND');try{localStorage.setItem(RESULT_ORDER_KEY,orderId)}catch{}const{data:{session}}=await client.auth.getSession();if(!session){location.replace(`../login/?order_id=${encodeURIComponent(orderId)}`);return}const{data,error}=await client.functions.invoke(config.contractResultFunction||'contract-result',{body:{order_id:Number(orderId)}});if(error||!data?.result)throw new Error(error?.context?.json?.message||'RESULT_NOT_FOUND');window.proverjdoGoal?.('payment_success',{product:'document_check',product_id:'contract_check_490',order_id:String(orderId),amount:490});history.replaceState({},document.title,`${location.pathname}?order_id=${encodeURIComponent(orderId)}`);try{localStorage.removeItem(RESULT_ORDER_KEY)}catch{}renderPaidResult(data)}catch(error){byId('result-status')?.classList.add('hidden');const box=byId('result-error');if(box){box.textContent=error?.message==='ORDER_NOT_FOUND'?'Не удалось определить оплаченный заказ.':'Не удалось открыть полный результат. Проверьте, что вошли с той же почтой, которую указали при оплате.';box.classList.remove('hidden')}}}
  setupCheck();setupScan();setupPayment();setupProverjdoLogin();setupPaidResult();
})();
