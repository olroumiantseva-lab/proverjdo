(() => {
  ...
  function renderParagraphs(node,text){if(!node)return;node.replaceChildren();const parts=String(text||'').split(/\n\s*\n/).map(v=>v.trim()).filter(Boolean);for(const part of parts){const p=document.createElement('p');p.textContent=part;p.style.margin='0 0 14px';node.append(p)}if(!parts.length){const p=document.createElement('p');p.textContent='Разбор готов.';node.append(p)}}
  ...
  function renderRisks(values){const box=byId('explain-result-risks');if(!box)return;box.replaceChildren();for(const [i,item] of (Array.isArray(values)?values:[]).entries()){const card=document.createElement('article');card.className='risk-card paid-risk-card';const head=document.createElement('div');head.className='paid-risk-head';const n=document.createElement('span');n.className='risk-number';const level={low:'низкий риск',medium:'средний риск',high:'высокий риск'}[item.level]||`Риск №${i+1}`;n.textContent=`Риск №${i+1} · ${level}`;const h=document.createElement('h3');h.textContent=item.title||'Риск';head.append(n,h);const why=document.createElement('p');const ws=document.createElement('strong');ws.textContent='Почему это важно. ';why.append(ws,document.createTextNode(item.why||''));const consequence=document.createElement('p');const cs=document.createElement('strong');cs.textContent='Что может произойти. ';consequence.append(cs,document.createTextNode(item.consequence||''));const action=document.createElement('p');const as=document.createElement('strong');as.textContent='Что сделать. ';action.append(as,document.createTextNode(item.action||''));card.append(head,why,consequence,action);box.append(card)}if(!box.children.length){const p=document.createElement('div');p.className='panel';p.textContent='Отдельных существенных рисков по тексту не выявлено.';box.append(p)}}
  function renderPaid(data){const r=data.result||{};byId('explain-result-type').textContent=r.document_type||'Документ';byId('explain-result-summary').textContent=r.summary||'Разбор завершён.';renderParagraphs(byId('explain-result-legal'),r.legal_analysis||r.preview_analysis||r.summary||'');byId('explain-result-meaning').textContent=r.what_it_means||'';renderRisks(r.risks);fillList('explain-result-actions',r.actions,'Обязательных действий не найдено.');fillList('explain-result-deadlines',r.deadlines,'Явных сроков не найдено.');fillList('explain-result-money',r.money,'Денежных условий не найдено.');fillList('explain-result-questions',r.questions,'Дополнительных вопросов не требуется.');const box=byId('explain-result-important');box.replaceChildren();for(const [i,item] of (Array.isArray(r.important_points)?r.important_points:[]).entries()){const card=document.createElement('article');card.className='risk-card paid-risk-card';const head=document.createElement('div');head.className='paid-risk-head';const n=document.createElement('span');n.className='risk-number';n.textContent=`Пункт №${i+1}`;const h=document.createElement('h3');h.textContent=item.title||'Важный пункт';head.append(n,h);const p=document.createElement('p');p.textContent=item.explanation||'';const a=document.createElement('p');const strong=document.createElement('strong');strong.textContent='Что сделать. ';a.append(strong,document.createTextNode(item.action||''));card.append(head,p,a);box.append(card)}if(!box.children.length){const p=document.createElement('div');p.className='panel';p.textContent='Отдельных важных пунктов не найдено.';box.append(p)}byId('explain-result-status').classList.add('hidden');byId('explain-result-content').classList.remove('hidden')}
